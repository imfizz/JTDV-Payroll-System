...
Class Payroll
{
        ...
        {   
            ...
            {                                                       //loop process all employee
                ...
                if($countRowa > 0) 
                {
                    ...
                    $sqlpagibig="SELECT * FROM pagibig_tbl WHERE name = 'pagibig';";
                    $stmtpagibig = $this->con()->prepare($sqlpagibig);
                    $stmtpagibig->execute();
                    $userpagibig = $stmtpagibig->fetch();
                        $pagibig = $userpagibig->amount;
                        
                    $sqlphilhealth="SELECT * FROM philhealth WHERE name = 'philhealth';";
                    $stmtphilhealth = $this->con()->prepare($sqlphilhealth);
                    $stmtphilhealth->execute();
                    $userphilhealth = $stmtphilhealth->fetch();
                        $philhealth = ($msc * $userphilhealth->amount) / 2;
    ...
    public function deletecashadv($logid)
    {
        ...
        $sql = "UPDATE cashadvance SET isDeleted = 'true' WHERE id = ?;";
        $stmt = $this->con()->prepare($sql);
        $stmt->execute([$logid]);
        $countrow = $stmt->rowCount();
        if($countrow > 0) {
    ...
    {
        $sql="SELECT cashadvance.id, cashadvance.date, cashadvance.amount, cashadvance.status, cashadvance.isDeleted, employee.firstname, employee.lastname FROM cashadvance INNER JOIN employee ON cashadvance.empId = employee.empId WHERE cashadvance.isDeleted IS NULL;";
        $stmt = $this->con()->prepare($sql);
        $stmt->execute();
        while($row = $stmt->fetch()){
        if($row->status == NULL){
